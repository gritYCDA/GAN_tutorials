...
class CycleGAN_Generator(nn.Module):
    def __init__(self, input_nc: int=3, output_nc: int=3, ngf: int=64):
        ...
        model.append(conv(input_nc, ngf, 7, stride=1, pad=0, bias=True, norm='in', activation='relu'))
        model.append(conv(ngf, ngf*2, 3, stride=2, pad=1, bias=True, norm='in', activation='relu'))
        model.append(conv(ngf*2, ngf*4, 3, stride=2, pad=1, bias=True, norm='in', activation='relu'))
        for i in range(n_blocks):
            model.append(ResnetBlock(ngf))
        model.append(deconv(ngf*4, ngf*2, 3, stride=2, pad=1, output_padding=1, bias=True, norm='in', activation='relu'))
        model.append(deconv(ngf*2, ngf, 3, stride=2, pad=1, output_padding=1, bias=True, norm='in', activation='relu'))
        model.append(nn.ReflectionPad2d(3))
        model.append(conv(ngf, output_nc, 7, stride=1, pad=0, bias=False, norm=None, activation='tanh'))
        ### END YOUR CODE
...
class ResnetBlock(nn.Module):
    def __init__(self, dim: int):
        ...
        model = [
            ...
            conv(256, 256, 3, stride=1, pad=0, bias=True, norm='in', activation='relu'),
            nn.ReflectionPad2d(1),
            conv(256, 256, 3, stride=1, pad=0, bias=True, norm='in', activation=None)
        ]
...
class CycleGAN_Discriminator(nn.Module):
    def __init__(self, input_nc: int=3, ndf: int=64):
        ...
        model.append(conv(input_nc, ndf, 4, stride=2, pad=1, bias=False, norm=None, activation='lrelu'))
        model.append(conv(ndf, ndf * 2, 4, stride=2, pad=1, bias=True, norm='in', activation='lrelu'))
        model.append(conv(ndf * 2, ndf * 4, 4, stride=2, pad=1, bias=True, norm='in', activation='lrelu'))
        model.append(conv(ndf * 4, ndf * 8, 4, stride=1, pad=1, bias=True, norm='in', activation='lrelu'))
        model.append(conv(ndf * 8, 1, 4, stride=1, pad=1, bias=False, norm=None, activation=None))
        ### END YOUR CODE
...
class CycleGAN_Solver():
    def __init__(self, lr: float=0.0002, batch_size: int=1, num_workers: int=1):
        ...
        self.optimizerG = torch.optim.Adam(itertools.chain(self.netG_A2B.parameters(), self.netG_B2A.parameters()), lr=lr, betas=(0.5, 0.999))
        self.optimizerD_A = torch.optim.Adam(self.netD_A.parameters(), lr=lr, betas=(0.5, 0.999))
        self.optimizerD_B = torch.optim.Adam(self.netD_B.parameters(), lr=lr, betas=(0.5, 0.999))
        ### END YOUR CODE
        ...
        ### YOUR CODE HERE (~ 1 line)
        self.trainloader, self.testloader = create_dataloader('summer2winter', batch_size=batch_size, num_workers=num_workers)
        ### END YOUR CODE
    ...
    def train(self, epochs: int=100):
        ...
        for epoch in range(epochs):
            for iter, (real_A, real_B) in enumerate(self.trainloader):
                ...
                lossD_A_real = self.criterion_GAN(pred_real, real_label.expand_as(pred_real))
                pred_fake = self.netD_A(fake_B.detach())
                lossD_A_fake = self.criterion_GAN(pred_fake, fake_label.expand_as(pred_fake))
                ...
                lossD_B_real = self.criterion_GAN(pred_real, real_label.expand_as(pred_real))
                pred_fake = self.netD_B(fake_A.detach())
                lossD_B_fake = self.criterion_GAN(pred_fake, fake_label.expand_as(pred_fake))
